...
export const home = () => {
    ...
    const generateFigureContent = ({bride}) => {
        ...
        // Untuk laki-laki ambil kata kedua (Nuh), untuk perempuan tetap kata pertama (Ajeng)
        const brideDisplayName = brideLName.split(' ')[1] || brideLName.split(' ')[0]; // Ambil "Nuh" dari "Muhammad Nuh Abdul Hamid"
        const prideDisplayName = bridePName.split(' ')[0]; // Tetap ambil "Ajeng" dari "Ajeng Widia Sari"
        
        return `
            <img src="${coupleImage}" alt="couple animation">
            <figcaption>
                ${brideDisplayName} & ${prideDisplayName}
            </figcaption>`;
    };
